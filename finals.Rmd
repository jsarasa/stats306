...
combined %>%
  ...
  labs(title = "How do UN Population projections compare to the previous\nrevision? World",
       ...
       x = "Year",
       y = "Projection",
       caption = "Data Source: UN, World Population Prospects 
...
population_projections |> 
  ...
  labs(title = "UN Population Projections as of 2024",
       subtitle = "Population projection from the UN World Population 
...
estimates |> rbind(mediums) |> 
  filter (region_subregion_country_area %in% c("China", "United States of America"), year >= start_year, year<= current_year) |> # Filter out the roles
  select (region_subregion_country_area, year, rate_of_natural_change_per_1000, infant_morality_rate_infant_deaths_per_1000_births) |> # Filter out the columns
...
estimates |> 
  filter(region_subregion_country_area %in% c("High-and-upper-middle-income countries", "Low-and-Lower-middle-income countries", "High-income countries", "Low-and-middle-income countries", "Middle-income countries", "Upper-middle-income countries", "Lower-middle-income countries", "Low-income countries", "No income group available"), year >= start_year, year<= current_year) |>
  ...
  summarize(AverageSexRatio = mean(sex_ratio_at_birth_males_per_100_female_births, na.rm = TRUE, count = n(), .group = 'drop')) |>

# Plot, bar chart shows distribution
ggplot(aes(x = reorder(region_subregion_country_area, - AverageSexRatio), y = AverageSexRatio, fill = region_subregion_country_area)) + # Order the bars by decreasng average sex ratio to better visualize the highest and the lowest
