...
graph6data <- graph6data %>% 
  mutate(total_population = ifelse(region_subregion_country_area == "Northern America", total_population - sum(total_population[region_subregion_country_area == "USA"], na.rm = TRUE), total_population)) %>%
  group_by(year) %>%
...
graph6data %>%
  ...
  labs(
    ...
    subtitle = "Comparing Population Proportions in 1950, 2020, and 2100",
    fill = "Region",
    x = "",
    y = "Proportion (%)"
  ) +
