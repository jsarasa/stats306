...
# total_pop_january_thousands and pop_2100 both have strong right skews, so we will use log transformations for both
...
predict_data |> left_join(mediums2100, join_by(region_subregion_country_area))  |> 
  ...
  mutate(diff = mlr_pop - undata, 
         ...
         sst = sum(st),
         r_sq = 1 - (ssr / sst)) |> select(-sr, -ssr, -st, -sst) # calculating R^2 between UN model and predicted
```
