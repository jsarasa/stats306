...
**Solution:** To clean and/or transform the data we performed the following steps shown in the code chunks below. 

- Step 1: we created a separate database called "est_values" that includes of all of the columns from estimates that are numbers, but the system recognizes these columns as characters, therefore, we converted them to numeric type using 'as.double'. We did this by mutating across the columns of character type and converting them to double type.
    - converted text values to numeric on the columns that are of character type. 
- Step 2: using the original estimates database we convert the column "Index" into a double so that it could act as a key column when we later merged the dataframes.
- Step 3: rewrite the dataframe to only include the following columns: (1) "Index" which is numeric, (2) "Region, subregion, country or area" which categorical type, and (3) "Type" which is categorical type. That is, we essentially only kept/selected the relevant columns.
- Step 4: use a full join to join the estimates table with the dataframe that had all of the numeric values. We joined them by the key "Index" column that we created above in step 2. Then, we filtered the rows to only include/keep the rows that do NOT have Type value as 'Label/Seperator' and thus removed any irrelevant rows. 
- Step 5-8: we repeated steps 1-4 in these steps for the mediums dataset
- Step 9: rename columns names to make it more readable by removing spaces and making all of the column names only lowercase.
- Step 10: select only columns that we use in our replication and EDA.


```{r}
# Step 1: Convert text values to numeric on the columns that need this transformation
est_values <- estimates |> 
  select(Index, Year:last_col()) |>
  mutate(across(where(is.character), as.double, .names = '{col}')) 

# Step 2: creates key column of Index and converts to double
estimates$Index <- estimates$Index |> as.double() 

# Step 3: select only relevant columns from original database 
# And remove any redundant columns
estimates <- estimates |> 
  select(Index, `Region, subregion, country or area *`, Type)

# Step 4; Full join & remove irrelevant rows
# Filter to keep rows that do NOT have Type value as 'Label/Separator'
estimates <- estimates |> 
  full_join(est_values, join_by(Index)) |> 
  filter(Type != "Label/Separator") # only includes relevant data in Estimates

# Step 5: replicated the above steps for the mediums dataset
med_values <- mediums |>
  select(Index, Year:last_col()) |>
  mutate(across(where(is.character), as.double, .names = '{col}')) 
...
# Step 8: Full join
mediums <- mediums |> 
  full_join(med_values, join_by(Index)) |> 
  filter(Type != "Label/Separator")
  
# Step 9: renaming columns to make it more readable
...
# Step 10: selecting only columns that we use in our replication and EDA
estimates <- estimates |> 
  select(index, year, region_subregion_country_area, type, total_pop_january_thousands, 
         total_pop_july_thousands, rate_of_natural_change_per_1000, births_thousands, 
         total_fertility_rate_live_births_per_woman, 
         sex_ratio_at_birth_males_per_100_female_births, total_deaths_thousands, 
         total_life_expectancy_at_birth_years, male_life_expectancy_at_birth_years, 
         female_life_expectancy_at_birth_years, female_life_expectancy_at_age_15_years, 
         infant_morality_rate_infant_deaths_per_1000_births, 
         live_births_surviving_to_age_1_thousands, 
         `deaths_under_age_5_per_1,000_live_births`, 
         total_male_mortality_before_age_40_per_1000_births, 
         total_mortality_before_age_60_per_1000_births, net_num_migrants_thousands, 
         net_migration_rate_per_1000, crude_birth_rate_per_1000_pop, 
         crude_death_rate_deaths_per_1000_population)
...
ggplot(aes(x = year, y = total_pop_july_thousands / 1e6, color = region_subregion_country_area)) +
  ...
  labs(title = "Population, 1950 - 2100",
       subtitle = "Projection from 2024 onwards are based on the UN's medium scenario.",
       x = "Year",
       y = "Population (in billions)",
       color = "Country") +
  scale_color_manual(values = c("China" = "darkblue", "India" = "darkred"))
```
...
```{r}
estimates %>%
  select(net_num_migrants_thousands, 
         year, 
         region_subregion_country_area) %>%
  filter(!is.na(year)) %>%
...
graph6data <- graph6data %>% 
  ...
  mutate(region_subregion_country_area = 
           ifelse(region_subregion_country_area == "Northern America", 
                                                   "Northern America (excluding USA)", 
                  region_subregion_country_area))
...
estimates |> rbind(mediums) |> 
  ...
  select (region_subregion_country_area, year, rate_of_natural_change_per_1000, infant_morality_rate_infant_deaths_per_1000_births) |>

  
#Plot
ggplot(aes(x = infant_morality_rate_infant_deaths_per_1000_births, 
           y = rate_of_natural_change_per_1000, color = region_subregion_country_area)) + 
  geom_point() +
...
# Order the bars by decreasing average sex ratio to better visualize the highest and the lowest
ggplot(aes(x = reorder(region_subregion_country_area, - AverageSexRatio), 
  ...
  theme_minimal() +
  coord_cartesian(ylim = c(100, 110)) + # limit y-axis to better display the differences
...
australia_data <- estimates %>%
  ...
  mutate(age_group = case_when(
    age_group == "infant_morality_rate_infant_deaths_per_1000_births" ~ 
                 "Infant Mortality (Per 1,000 Births)",
    age_group == "deaths_under_age_5_per_1,000_live_births" ~ 
                  "Under Age 5 (Per 1,000 Births)",
    age_group == "total_male_mortality_before_age_40_per_1000_births" ~ 
                 "Total Mortality <40 (Per 1,000)",
    age_group == "total_mortality_before_age_60_per_1000_births" ~ 
                 "Total Mortality <60 (Per 1,000)",
    TRUE ~ age_group
  ))
...
- **Question:** How do Live Births Surviving numbers in Australia and the United States compare across decades?
...
estimates |>
  ...
  ggplot(aes(x = year, 
             ...
             group = region_subregion_country_area, color = region_subregion_country_area)) +
  geom_line(size = 1.2) +
