...
3. Diagram 3: Shalini Asokkumar
  - **Question:** How have birth and death rates in the BRICs countries changed over time, and how are they predicted to continue? 
  - **Interpretation of Diagram:** Befo
...
estimates |> 
  ...
  summarise(`Average Birth Rate` = mean(births_thousands, na.rm = T), 
            `Average Death Rate` = mean(total_deaths_thousands, na.rm = T)) |>
  pivot_longer(cols = c("Average Birth Rate", "Average Death Rate"), names_to = "rate_type", values_to = "averages") |>
  ggplot(aes(x = year, y = averages, color = rate_type)) +
  scale_color_manual(values=c('chartreuse4','#DAB1DA'))+
  geom_line() +
  facet_wrap(~region_subregion_country_area) +
  labs(title = "Population growth versus Industrial Development", 
       subtitle = "From 1950 - 2023", 
       x = "Year", 
